...
- **BranchTree** — выдвижная панель справа, переключается кнопкой GitBranch в заголовке чата. Работает даже на пустых чатах (пустое состояние). Кнопка-крестик для закрытия
...
- **Session Prompt** — кастомный system prompt для этой сессии + прикрепление файлов `.txt/.md` (читаются клиентом через FileReader, содержимое добавляется в textarea с заголовком `# filename`). Textarea занимает максимум доступного пространства (`flex-1`)
