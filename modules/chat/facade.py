    ...
    try:
        ...
        for o in offers:
            ...
            if o.get("in_stock") is True:
                ...
                # Сайт учёт остатков не ведёт (manage_stock выключен), поэтому
                # «в наличии» здесь было бы утверждением того, чего мы не знаем.
                stock = "наличие уточняется"
        ...
        block = (
            ...
            + "\n".join(lines)
            + "\n\nПравила ответа: цену, артикул и наличие бери ТОЛЬКО отсюда, не выдумывай. "
            "Позицию с пометкой «НЕТ в наличии» так и назови — нет в наличии — и сразу предложи "
            "из этого же списка технически подходящую замену (близкий номинал, то же напряжение "
            "и тип контактов), прямо сказав, что это замена и чем она отличается. "
            "«наличие уточняется» значит, что позиция в каталоге есть, но остаток не подтверждён — "
            "так и скажи, не выдавай за наличие. Если подходящего нет вообще — честно скажи "
            "«не нашёл» и предложи передать запрос менеджеру."
        )
        if manager_ctx:
            block += (
