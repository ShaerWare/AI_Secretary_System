...
from modules.core.service import user_service, user_session_service
...
async def admin_update_profile(
    ...
    result = await user_service.update_profile(user.id, display_name=request.display_name)
    if not result:
        raise HTTPException(status_code=404, detail="User not found")

    await audit_service.log(action="update", resource="profile", user_id=user.username)
    return result
...
async def admin_change_password(
    ...
    success = await user_service.update_password(user.id, request.new_password)
    if not success:
    ...
    await audit_service.log(action="update", resource="password", user_id=user.username)
    return {
...
async def list_sessions(user: User = Depends(get_current_user)):
    ...
    sessions = await user_session_service.get_active_for_user(user.id)
    return sessions
...
async def delete_session(
    ...
    if not level_gte(user.permissions.get("users", ""), "manage"):
        db_session = await user_session_service.get_by_jti(jti)
