...
async def _check_widget_owner_or_admin(instance_id: str, user: User) -> dict:
    ...
    instance = await widget_instance_service.get_instance(
...
async def admin_get_widget_config(user: User = Depends(require_permission("channels", "view"))):
    ...
    instance = await widget_instance_service.get_instance("default")
    if instance:
...
async def admin_save_widget_config(
    ...
    await config_service.set_widget(config)

    # Also save to 'default' widget instance
    existing_instance = await widget_instance_service.get_instance("default")
    if existing_instance:
        await widget_instance_service.update_instance("default", **config)
    else:
        # Create default instance if it doesn't exist
        await widget_instance_service.create_instance(
...
async def admin_list_widget_instances(
    ...
    owner_id, ws_id = workspace_context(user, "channels")
    instances = await widget_instance_service.list_instances(
        enabled_only=enabled_only, owner_id=owner_id, workspace_id=ws_id
    )

    # Enrich with share info
    instance_ids = [i["id"] for i in instances]
    share_counts = await resource_share_service.get_share_counts(RESOURCE_TYPE_WIDGET, instance_ids)
    shared_with_me = {}
...
async def admin_create_widget_instance(
    ...
    instance = await widget_instance_service.create_instance(**kwargs)
...
async def admin_update_widget_instance(
    ...
    owner_id, ws_id = workspace_context(user, "channels")
    existing = await widget_instance_service.get_instance(
        instance_id, owner_id=owner_id, workspace_id=ws_id
    )
    if not existing:
        raise HTTPException(status_code=404, detail="Widget instance not found")
    ...
    instance = await widget_instance_service.update_instance(instance_id, **kwargs)
...
async def admin_delete_widget_instance(
    ...
    owner_id, ws_id = workspace_context(user, "channels")
    success = await widget_instance_service.delete_instance(
        instance_id, owner_id=owner_id, workspace_id=ws_id
    )
    if not success:
        raise HTTPException(status_code=404, detail="Widget instance not found")
    ...
    # Clean up shares
    await resource_share_service.remove_all_shares(RESOURCE_TYPE_WIDGET, instance_id)
...
async def get_widget_shares(
    ...
    shares = await resource_share_service.get_shares(RESOURCE_TYPE_WIDGET, instance_id)
