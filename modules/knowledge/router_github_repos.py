...
from modules.knowledge.service import (
    github_repo_project_service,
    knowledge_collection_service,
...
async def list_projects(user: User = Depends(require_permission("wiki", "view"))):
    ...
    projects = await github_repo_project_service.list_projects(ws_id)
...
async def create_project(
    ...
    # Check for duplicate
    existing = await github_repo_project_service.get_by_repo(data.github_owner, data.github_repo)
    if existing:
        raise HTTPException(
            status_code=409,
    ...
    project_dict = await github_repo_project_service.create_project(**create_kwargs)
    ...
    try:
        ...
        await github_repo_project_service.update_sync_status(
            project_id,
            status="idle",
    ...
    return {
        "status": "ok",
        "project": (await github_repo_project_service.list_projects(ws_id))[0]
        if len(await github_repo_project_service.list_projects(ws_id)) == 1
        else project_dict,
...
async def update_project(
    ...
    result = await github_repo_project_service.update_project(project_id, ws_id, **kwargs)
    if not result:
        raise HTTPException(status_code=404, detail="Project not found")
...
async def delete_project(
    ...
    await github_repo_project_service.delete_project(project_id, ws_id)
...
async def sync_project(
    ...
    project_orm = await github_repo_project_service.get_project(project_id, ws_id)
    if not project_orm:
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    await github_repo_project_service.update_sync_status(project_id, status="syncing")
