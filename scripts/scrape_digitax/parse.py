...
SKIP_FILENAMES = {
    "ru-sbup-seo": [
        # Keep only learning content: forum threads (incl. the «SEO от А до Я»
        # textbook), the SEO wiki, and articles. Everything else on sbup.com is
        # SEO-tool output (alexa-rank/pagerank/tic rating tables, site-audit
        # dumps of random external domains, whois/port-scanner utilities) —
        # no educational value, pure RAG noise.
        re.compile(r"^(?!seo-forum__|wiki|seo-articales)"),
    ],
...
def parse_forum_smf(filepath: Path, slug: str) -> dict | None:
    ...
    posts = []
    # Each SMF post is `windowbg > post_wrapper > (poster | postarea>inner)`.
    # Select post_wrapper only: a union with the windowbg ancestor matched every
    # post twice. Fall back to windowbg when a theme omits post_wrapper.
    post_els = forumposts[0].xpath('.//div[contains(@class, "post_wrapper")]')
    if not post_els:
        post_els = forumposts[0].xpath(
            './/div[contains(@class, "windowbg") and .//div[contains(@class, "postarea")]]'
        )
    for post_el in post_els:
        ...
        # Body — the message text lives in `.postarea .inner`. Target it
        # directly: a bare `contains(@class, "post")` also matches the `.poster`
        # / `.postarea` author sidebar (rank, post count) which sits before the
        # message in document order, so body_els[0] would be poster metadata.
        body_els = post_el.xpath(
            './/div[contains(@class, "postarea")]//div[contains(@class, "inner")]'
