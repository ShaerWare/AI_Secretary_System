...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "updated", sa.DateTime(), server_default=sa.text("(CURRENT_TIMESTAMP)"), nullable=False
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("ix_kanban_tasks_created_by"), "kanban_tasks", ["created_by"], unique=False
    )
    ...
    op.create_index(
        op.f("ix_kanban_checklist_items_task_id"),
        "kanban_checklist_items",
        ["task_id"],
        unique=False,
    )
