...
**Claude Code Web UI** (`app/routers/claude_code.py`): WebSocket-based terminal for Claude Code CLI. WebSocket at `/admin/claude-code/ws?token=<jwt>` streams structured events (text_delta, thinking_delta, tool_use_start, tool_result, turn_complete). REST endpoints for session management (list/get/delete) and project management (list/create/delete). `ClaudeCodeSession` model tracks sessions in DB. `ClaudeCodeProject` model (`claude_code_projects` table) stores project directories — supports both local paths and SSH remote hosts (`type`: "local" or "ssh"; SSH fields: `ssh_host`, `ssh_user`, `ssh_port`, `ssh_key_path`). SSH projects wrap the claude CLI command in an SSH call to the remote host. Built-in default directories (`/root`, `/opt/ai-secretary`, `/tmp`) are prepended to DB projects. Project CRUD: `GET/POST/DELETE /admin/claude-code/projects`. `start` WS command accepts optional `project_id`. One active WebSocket per user. Admin-only. Frontend: `useClaudeCode` composable (`projectId` ref), dynamic project switcher dropdown in `ChatView.vue` (replaces hardcoded directories) with inline add/delete forms and SSH field support. Repository: `ClaudeCodeProjectRepository` in `db/repositories/claude_code_project.py`, manager: `AsyncClaudeCodeProjectManager` in `db/integration.py`.
