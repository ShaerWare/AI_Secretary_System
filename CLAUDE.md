...
**Middleware** (`app/`): `cors_middleware.py` (CORS with configurable origins via `CORS_ORIGINS`), `rate_limiter.py` (slowapi, per-endpoint limits via `RATE_LIMIT_*` env vars), `security_headers.py` (X-Frame-Options, CSP, etc. via `SECURITY_HEADERS_ENABLED`). All registered in `orchestrator.py` startup. **Uvicorn runs with `proxy_headers=True` and `forwarded_allow_ips="*"`** so that `request.base_url` respects `X-Forwarded-Proto` from nginx — required for widget embed code to generate `https://` API URLs behind a reverse proxy.
...
**Widget test chat**: Widget instances can be tested live from the admin panel. `app/routers/chat.py` accepts an optional `widget_instance_id` parameter on streaming endpoints, which overrides LLM/TTS settings to match the widget's config. Frontend in `WidgetView.vue` test tab. The embeddable widget (`web-widget/ai-chat-widget.js`) performs a runtime enabled check via `GET /widget/status` (public, no auth) — if the instance is disabled, the widget icon won't render on the site. When embedded in the admin panel, the widget auto-attaches JWT from `localStorage('admin_token')` for authenticated chat. **Widget hidden on Chat page**: `App.vue` watches `route.name` and polls for `.ai-chat-widget` DOM element; adds `.ai-chat-widget-hidden` class when on `/chat` route to prevent overlap with the built-in chat UI.
...
**Chat markdown rendering**: Assistant and user messages render markdown via `marked` + `DOMPurify`. Custom `.chat-markdown` CSS in `main.css` styles headings, code blocks, lists, tables, blockquotes, links using HSL theme variables. Fenced code blocks rendered with `.code-block-container` wrapper: header bar with language label + copy button + "open in artifact viewer" button (SVG icons, `data-*` attributes for delegated click handling since content is `v-html`). DOMPurify configured with `ADD_ATTR` for `data-artifact-*` and `ADD_TAGS` for SVG elements.

**Chat message layout** (Claude.com style): Flat document-flow layout — both user and assistant messages left-aligned with avatar on left, no bubble backgrounds, no `max-width` cap. CSS classes: `.claude-message` (flex row), `.claude-avatar` / `.claude-avatar-user` / `.claude-avatar-assistant` (small circle with initial or Bot icon), `.claude-message-content` (flex-1). Messages container uses `.claude-messages-container` for centered column (`max-w: 48rem`). User messages differentiated by subtle bottom border (`.claude-message-user`).

**Artifact viewer**: `ArtifactPanel.vue` — resizable right panel (same slot as branch tree / settings, mutually exclusive). Two tabs: Code (raw source, monospace `<pre><code>`) and Preview (rendered via `marked` + `DOMPurify` with `.chat-markdown` styles). Opened via delegated click handler on code block "open in viewer" buttons. State: `showArtifact`, `activeArtifact` (id, language, code, messageId). Resizable via `useResizablePanel('chat-artifact-width', 500, 300, 800, 'left')`. Mobile: fullscreen overlay with backdrop.
...
**Kanban/Tasks** (`app/routers/kanban.py`): Project task management board with Gantt roadmap. `KanbanTask` model with status (`todo`/`in_progress`/`review`/`done`), assignee, dates, tags (JSON), `is_private`, `position` for drag-reorder. `KanbanTaskDependency` (blocker → dependent), `KanbanChecklistItem` (per-task checklists). `KanbanTaskStatus` enum (includes legacy `draft` — mapped to `todo` in UI). `POST /tasks` accepts optional `status` field (default `todo`). 10 endpoints: CRUD, reorder, dependency management, checklist items. Frontend: `KanbanView.vue` — ChatView-style layout with resizable/collapsible sidebar listing kanban boards (local + GitHub projects), `+` button creates new board. Main area: `KanbanBoard.vue` (4 columns, drag & drop, per-column `+` button to create task in that status), `KanbanCard.vue`, `KanbanCardDetail.vue` (modal), `KanbanTaskForm.vue` (accepts `initialStatus` prop), `KanbanRoadmap.vue` (Gantt-style timeline), `KanbanStatusBadge.vue`. Migration: `scripts/migrate_kanban.py`. **GitHub sync**: `app/services/github_kanban_sync.py` — bidirectional sync between GitHub Issues and kanban tasks. `sync_all_issues(project_id)` fetches all issues and upserts into kanban; `push_status_to_github()` pushes status changes back; `handle_issue_webhook()` processes GitHub webhook events. Periodic auto-sync runs every 15 minutes in `orchestrator.py` (`_periodic_kanban_sync()`) for projects with `sync_enabled=True` and a configured token.

**Claude Code Web UI** (`app/routers/claude_code.py`): WebSocket-based terminal for Claude Code CLI. WebSocket at `/admin/claude-code/ws?token=<jwt>` streams structured events (text_delta, thinking_delta, tool_use_start, tool_result, turn_complete). REST endpoints for session management (list/get/delete/patch) and project management (list/create/delete). `ClaudeCodeSession` model tracks sessions in DB with `chat_session_id` (FK → chat_sessions, nullable) and `kanban_task_id` (FK → kanban_tasks, nullable) for linking CC sessions to parent chats and kanban tasks. `events_json` column stores transcript as JSON blob. `to_dict()` returns full data including transcript; `to_summary()` returns lightweight dict without `events_json` (used in lists). `ClaudeCodeProject` model (`claude_code_projects` table) stores project directories — supports both local paths and SSH remote hosts (`type`: "local" or "ssh"; SSH fields: `ssh_host`, `ssh_user`, `ssh_port`, `ssh_key_path`). SSH projects wrap the claude CLI command in an SSH call to the remote host. Built-in default directories (`/root`, `/opt/ai-secretary`, `/tmp`) are prepended to DB projects. Project CRUD: `GET/POST/DELETE /admin/claude-code/projects`. Session persistence: `save_transcript` WS action writes messages to `events_json`; `GET /sessions/{id}/transcript` returns full session with transcript; `GET /sessions/by-chat/{chat_session_id}` lists CC sessions linked to a chat; `PATCH /sessions/{id}` updates title/kanban_task_id/chat_session_id; `GET /admin/kanban/tasks/{id}/cc-sessions` lists CC sessions for a kanban task. `start` WS command accepts optional `project_id`, `chat_session_id`, `kanban_task_id`. One active WebSocket per user. Admin-only. Frontend: `useClaudeCode` composable (`projectId`, `chatSessionId`, `kanbanTaskId` refs; `loadSession()` fetches transcript and restores state; auto-saves transcript on done/error/aborted). ChatView sidebar shows standalone CC sessions in a "Claude Code" header section + indented CC sub-sessions under each parent chat; each session has a colored status dot (`getCcIndicator(session)` → `{bg, pulse, title}`): green pulsing = active processing, yellow pulsing = waiting for user reply (isProcessing=false but status=active), green dim = active (other session), gray = completed, red = error, orange = aborted. Kanban task picker dropdown (ListChecks icon) in CC toolbar. `KanbanCardDetail.vue` shows linked CC sessions at bottom. Migration: `scripts/migrate_cc_persistence.py` (drops and recreates `claude_code_sessions` table). Repository: `ClaudeCodeProjectRepository` in `db/repositories/claude_code_project.py`, manager: `AsyncClaudeCodeProjectManager` in `db/integration.py`.
...
**Resource sharing (bots/widgets/WhatsApp)**: `ResourceShare` model (`resource_shares` table) — polymorphic sharing for `bot_instance`, `widget_instance`, `whatsapp_instance`. Single table with `resource_type` discriminator, `resource_id` (String(50)), `user_id` (FK), `permission` ("view"/"edit"), `shared_by`, `shared_at`. Unique constraint on `(resource_type, resource_id, user_id)`. `ResourceShareRepository` in `db/repositories/resource_share.py`, `AsyncResourceShareManager` in `db/integration.py` (singleton `async_resource_share_manager`). Instance repositories (`bot_instance.py`, `widget_instance.py`, `whatsapp_instance.py`) include shared resources via subquery in `list_instances()` and `get_instance()` — non-admin users see own + NULL-owned + shared-with-me. Share CRUD endpoints on each router: `GET/POST /instances/{id}/shares`, `PUT/DELETE /instances/{id}/shares/{user_id}`, plus `GET /admin/telegram/shareable-users` (shared across all). Permission enforcement: "view" users get 403 on update/start/stop/restart; only owner/admin can delete or manage shares. On instance delete, `remove_all_shares()` cleans up. GDPR: explicit `DELETE FROM resource_shares WHERE user_id = :uid`. List responses enriched with `share_count`, `is_shared_with_me`, `share_permission`. Frontend: `ResourceShareDialog.vue` (reusable, prop-driven — takes `getShares`/`addShare`/`updatePermission`/`removeShare` callbacks), Share2 button in instance toolbars (hidden for NULL-owned), share indicator in sidebar lists. i18n: `resourceShare` key block (ru/en/kk). Demo: mock handlers in `telegram.ts`/`widget.ts`/`whatsapp.ts`. Migration: `alembic/versions/20260226_0001_add_resource_shares.py`.
...
- 14 resource tables have `workspace_id` FK (NOT NULL, default=1): `chat_sessions`, `bot_instances`, `widget_instances`, `whatsapp_instances`, `cloud_llm_providers`, `tts_presets`, `knowledge_documents`, `knowledge_collections`, `claude_code_sessions`, `claude_code_projects`, `faq_entries`, `kanban_tasks`, `kanban_projects`, `amocrm_config`
...
**Components** (`admin/src/components/`): Flat structure, no `ui/` subdirectory. UI state components (`ConfirmDialog`, `SearchPalette`, `ToastContainer`, `ThemeToggle`) driven by dedicated Pinia stores. `BranchTree.vue` / `BranchTreeNode.vue` for chat branching. `ArtifactPanel.vue` — code/preview viewer panel for chat code blocks. `CrmInboxAmoCRM.vue` — extracted amojo messenger (used as sub-tab in `CrmInbox.vue`). `charts/` for Chart.js wrappers.
