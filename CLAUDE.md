...
**Chat session prompts (roles)** (`modules/chat/service.py` `ChatPromptService`, table `chat_session_prompts`): Named prompts attached to a chat session — a set of "roles" the user can switch between on the fly without losing the conversation. Exactly one prompt per session is `is_active=True`; its `content` is mirrored into `ChatSession.system_prompt` on activation, so the existing prompt fallback chain in `modules/chat/facade.py` picks it up unchanged. Endpoints (gated by `chat:view` + `_check_write_access` — any user with write-access to the session can manage its prompts): `GET/POST /admin/chat/sessions/{id}/prompts`, `PATCH/DELETE .../prompts/{prompt_id}`, `POST .../prompts/{prompt_id}/activate`. Migration `scripts/migrate_chat_session_prompts.py` seeds one prompt per legacy session from existing `system_prompt`. UI is a chip-row + textarea in the "Промпт" settings tab of both admin and mobile `ChatView.vue`.
