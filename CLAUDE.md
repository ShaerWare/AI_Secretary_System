...
Code-pipeline «единый поиск позиции» для торгового ассистента (StalkerElectric). Модель `ProductOffer` (table `product_offers`) — единое структурное представление оффера из любого источника: `source` (`site`/`ekf`/`supplier`), `article`, `name`, `price`, `in_stock`, `lead_time_days`, `url` + `UniqueConstraint(source, source_key)` для upsert. `OfferService.replace_source_offers()` делает полный ре-синк одного источника; `OfferService.search()` детерминированно ранжирует (article_exact → article_partial → name_all → name_any; тай-брейк: in_stock, `SOURCE_PRIORITY` site<ekf<supplier, price) и **возвращает `[]`, если ничего не найдено — не выдумывает позиций** (жёсткое требование клиента реализовано архитектурно, а не промптом). Адаптеры населяют общую таблицу: `site_adapter.sync_site_offers()` (WooCommerce→офферы, ~30k товаров) и `suppliers/` (config-driven парсер прайсов поставщиков) готовы; EKF (партнёрский REST API `ekfgroup.com/api/v{N}`) встаёт в тот же интерфейс позже. **`modules/procurement/suppliers/`**: `registry.py` (per-supplier конфиги — формат/header_row/cols/currency/markup/stock_file, как SITES в scrape_digitax), `parser.py` (xlsx через openpyxl + `pdf_lines` через pdfplumber; `parse_number` для «2 453»/«1,86»/«5.3»; мерж остатков по артикулу), `adapter.py` (`sync_supplier`/`sync_all_suppliers` → `offer_service.replace_source_offers(..., scope_supplier=name)` — поставщики не затирают друг друга в source='supplier'). Прайсы из env `SUPPLIER_PRICES_DIR`. Наценка 20% (`DEFAULT_MARKUP_PCT`) единая, МегаЗаказ override (USD + скидка закупки 28%). **Дилерские (закупочные) цены поставщиков хранятся сырыми — при инъекции в чат гейтить видимость: только менеджерский/инженерный контекст, не клиентский виджет/бот.** Провайдер `claude_bridge` без tools → поиск = код-шаг, не агентный tool. Роутер `/admin/procurement/{status,search,sync,sync-suppliers}` (RBAC `sales`). Задача `procurement-site-sync` (daily 23:30 UTC). Перф: `search()` грузит офферы в память — приемлемо на фоне bridge-латентности; FTS при росте таблицы. Ещё не сделано: 1С .xls (категории-вперемешку) + светильники PDF (USD/МегаЗаказ) + rate_service (mig.kz) + pricing_engine (наценка/НДС/курс для КП).
