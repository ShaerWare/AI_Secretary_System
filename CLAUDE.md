...
**Global state in orchestrator.py** (~3700 lines, ~106 endpoints): This is the FastAPI entry point. It initializes all services as module-level globals, populates the `ServiceContainer`, and includes all routers. Legacy endpoints (OpenAI-compatible `/v1/*`) still live here alongside the modular router system.
...
**CRM Dataset (Knowledge Base Sync)**: Fetches all amoCRM data (pipelines + leads) and syncs as markdown documents into a dedicated RAG collection, making deals searchable by LLM. `app/services/crm_dataset_service.py` — pure functions: `build_pipeline_document()` generates per-pipeline markdown with deal sections (name, status, price, contacts, tags, timestamps); `build_summary_document()` generates aggregate stats + recent deals; `clean_crm_files()` cleans output dir (`data/crm-dataset/`). `app/services/amocrm_service.py` adds `get_all_leads_paginated()` (250/page until exhausted). API endpoints in `app/routers/amocrm.py`: `POST /admin/crm/dataset-sync` (full sync → auto-create "amocrm" collection with `base_dir="data/crm-dataset"` → reindex), `GET /admin/crm/dataset-status` (sync state), `DELETE /admin/crm/dataset` (admin-only clear).
