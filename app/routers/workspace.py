...
async def create_invite(
    ...
    invite = await async_workspace_manager.create_invite(
...
async def list_invites(
    ...
    return await async_workspace_manager.list_invites(user.workspace_id)
...
async def delete_invite(
    ...
    """Delete an invite."""
    deleted = await async_workspace_manager.delete_invite(user.workspace_id, invite_id)
    if not deleted:
...
async def get_invite_info(invite_code: str):
    ...
    info = await async_workspace_manager.get_invite_info(invite_code)
    if not info:
...
async def accept_invite(body: InviteAccept, request: Request):
    ...
    existing = await async_user_manager.get_by_username(body.username)
    if existing:
        raise HTTPException(status_code=409, detail="Username already taken")

    # Accept invite (create user + membership)
    result = await async_workspace_manager.accept_invite(
