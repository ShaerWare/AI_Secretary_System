...
async def get_config(user: User = Depends(require_permission("sales", "view"))):
    ...
    config = await async_woocommerce_manager.get_config()
    if not config:
        return {"config": None}
...
async def save_config(
    ...
    config = await async_woocommerce_manager.save_config(**kwargs)
...
async def test_woocommerce_connection(
    ...
    secrets = await async_woocommerce_manager.get_config_with_secrets()
    ...
    # Mark as connected
    await async_woocommerce_manager.save_config(is_connected=True)
...
async def disconnect_woocommerce(
    ...
    await async_woocommerce_manager.clear_credentials()
...
async def wc_dataset_sync(user: User = Depends(require_permission("sales", "edit"))):
    ...
    collection = await async_knowledge_collection_manager.get_by_slug(WC_COLLECTION_SLUG)
    if not collection:
    ...
    await async_woocommerce_manager.save_config(
