...
[![Android APK](https://img.shields.io/badge/Android_APK-v2.4-3DDC84?style=for-the-badge&logo=android&logoColor=white)](https://github.com/ShaerWare/AI_Secretary_System/releases/download/mobile-v2.4/ai-secretary-2.4.apk)
...
> **Важно:** При использовании облачных LLM провайдеров (Anthropic, Google, OpenAI и др.) все данные из запросов — **системные промпты, RAG-контекст из базы знаний, датасеты товаров/CRM и сообщения пользователей** — передаются третьим лицам. При большом трафике провайдер со временем получит практически всю вашу базу данных через RAG-фрагменты в промптах.

**Истинная приватность возможна только при использовании локальных моделей:**

| Режим | Конфиденциальность | Кто видит данные |
|-------|-------------------|------------------|
| **Локальный LLM** (vLLM + Qwen/Llama) | Полная | Только вы |
...
Для защиты конфиденциальных данных (каталоги товаров, клиентская база, know-how):
- Используйте `LLM_BACKEND=vllm` с `DEPLOYMENT_MODE=full`
- Используйте локальные embeddings (sentence-transformers) вместо облачных
- Для каналов с облачным LLM отключите RAG (`rag_mode: none`)

Подробнее: [Wiki — Data Privacy](https://github.com/ShaerWare/AI_Secretary_System/wiki/Data-Privacy)
...
📱 [**ai-secretary-2.4.apk**](https://github.com/ShaerWare/AI_Secretary_System/releases/download/mobile-v2.4/ai-secretary-2.4.apk) — последняя сборка (debug, не подписана для Play Store)

Все релизы: [github.com/ShaerWare/AI_Secretary_System/releases](https://github.com/ShaerWare/AI_Secretary_System/releases)

**Установка через adb:**
```bash
adb install -r ai-secretary-2.4.apk
```
